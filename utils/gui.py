...
class Button(Widget):
    ...
    def display(self):
        pygame.draw.rect(self.parent, self.ct_color,
                         (self.x - 3, self.y - 3, self.width + 6, self.height + 6 ))
        pygame.draw.rect(self.parent, self.bg_color,
    ...
    def click(self, coursor_position):
        if (self.coursor_on_widget(coursor_position)
                and not Widget.active_widget):
...
class Slider(Widget):
    ...
    def display(self, coursor_position=(None, None)):
        ...
        lower_circle_center = (
            self.x + outer_radius,
            self.y + self.height - outer_radius
            )
        sliding_circle_center = (
            self.x + outer_radius,
            min(lower_circle_center[1],
                max(coursor_position[1],
                    upper_circle_center[1]
                    )))

        pygame.draw.circle(self.parent, self.ct_color,
                           upper_circle_center, outer_radius + 2)
        pygame.draw.circle(self.parent, self.ct_color,
                           lower_circle_center, outer_radius + 2)
        pygame.draw.rect(self.parent, self.ct_color,
                         (self.x - 3, self.y + outer_radius - 3,
                          self.width + 6, self.height - self.width + 6))

        pygame.draw.circle(self.parent, self.sa_color,
                           upper_circle_center, inner_radius)
        pygame.draw.circle(self.parent, self.sa_color,
                           lower_circle_center, inner_radius)
        pygame.draw.rect(self.parent, self.sa_color,
                         (self.x, self.y + outer_radius,
                          self.width, self.height - self.width))

        pygame.draw.circle(self.parent, self.ct_color,
                           sliding_circle_center, outer_radius + 2)
        pygame.draw.circle(self.parent, self.bg_color,
                           sliding_circle_center, inner_radius)

        self.z = 10 * ((lower_circle_center[1] - sliding_circle_center[1]) / (
            lower_circle_center[1] - upper_circle_center[1])) - 5
    ...
    def click(self, coursor_position):
        if self.coursor_on_widget(coursor_position) and (
                not Widget.active_widget or Widget.active_widget == self):
            Widget.active_widget = self
...
class Canvas(Widget):
    def __init__(self, parent, x, y, width, height):
        ...
        pygame.draw.rect(self.parent, colors["black"],
                         (self.x - 3, self.y - 3, self.width + 6, self.height + 6))
        pygame.draw.rect(self.parent, colors["white"],
                         (self.x, self.y, self.width, self.height))
    ...
    def draw(self, coursor_position):
        if self.coursor_on_widget(coursor_position) and (
            ...
            if not self.busy:
                ...
                self.points.append(coursor_position)
                pygame.draw.circle(
                    self.parent, colors["black"], self.points[-1], 2)
                return [0, coursor_position]

            elif coursor_position != self.prev:
                self.add_point(coursor_position, self.prev)
                pygame.draw.circle(
                    self.parent, colors["black"], coursor_position, 2)
                pygame.draw.line(
                    self.parent, colors["black"], self.prev, coursor_position, 5)
                self.prev = coursor_position
        ...
        for done_point in progress:
            if self.points:
                if self.points[0] == (done_point[0], done_point[1]):
                    pygame.draw.circle(
                        self.parent, colors["green"], self.points[0], 2)
                    del self.points[0]
            if self.lines:
                if self.lines[0][1] == (done_point[0], done_point[1]):
                    pygame.draw.line(
                        self.parent, colors["green"], self.lines[0][0], self.lines[0][1], 5)
                    del self.lines[0]
    ...
    def clear(self):
        ...
        self.lines = []
        pygame.draw.rect(
            self.parent, colors["black"], (self.x - 3, self.y - 3, self.width + 6, self.height + 6))
        pygame.draw.rect(
            self.parent, colors["white"], (self.x, self.y, self.width, self.height))
