...
class Host():
    ...
    def send_data(self, data, canvas):
        if data:

            data = [(min((canvas.width),max(0, x - canvas.x)),
                     min((canvas.height),max(0, canvas.y + canvas.height - y)),
                     z) for [x,y,z] in data]
...
class PointHandler:
    ...
    def move_robot(self, canvas, z, host):
        if not self.special_active and not self.point_buffer:
            ...
            host.send_data(([[canvas.x, canvas.y, z + 50]]*3), canvas)
